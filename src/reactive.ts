...
function findCountries(query$: Observable<string>): Observable<string[] | null> {
  return query$.pipe(
...
export const useCountries = (ref: HTMLInputElement, time: number) => {
  const values = fromEvent(ref, "input").pipe(map((event) => event.target?.value), startWith(ref.value));
